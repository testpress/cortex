...
- **THEN** the system SHALL display an ended/unavailable notice — no action button
...
The system SHALL preserve the existing inline video-player experience for all TpStreams live sessions. No TpStreams-specific behaviour SHALL be altered by this change.
...
- **THEN** the system SHALL render the existing `CustomVideoPlayer` with the HLS stream URL, exactly as before
