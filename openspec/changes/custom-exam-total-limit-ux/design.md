...
- Disable adding new subjects if the remaining budget is less than the minimum required block size (5).
...
- **Button UX State:** 
  ...
  - If `blocks.isEmpty` and `remaining >= 5`: "+ Add Questionnaire"
  - If `blocks.isNotEmpty` and `remaining >= 5`: "+ Add More Questionnaires"
  - If `remaining < 5`: "Limit Reached ($usedLimit/$totalLimit)"

## Risks / Trade-offs

- **Risk:** The minimum block size is hardcoded as `5` in multiple places. If this changes, it might break the `< 5` logic.
  **Mitigation:** We will keep the hardcoded `5` logic consistent with the existing slider implementation but structure it cleanly so it can be refactored easily later.
