...
- [x] 1.2 Update the `+ Add Questionnaire` button logic to be disabled when `remainingQuota < 1`.
...
- [x] 2.3 Modify the `_saveBlock` method to strictly clamp `_noOfQuestions` to the `remainingQuota` to prevent the default (now 1) from bypassing the limit.
