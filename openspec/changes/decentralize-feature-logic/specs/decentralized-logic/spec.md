...
- **THEN** it imports the DTO from itself; no feature should reach into another feature package for internal types.

#### Scenario: Upcoming Test Feed
- **WHEN** the dashboard needs upcoming tests
- **THEN** the data comes from course-owned DTOs/mock data or `core` (never from `package:exams`), so the home surface stays self-contained.

### Requirement: Robust Exam Mocks
The `exams` package must provide a fully populated thermodynamics mock exam to keep the shell’s practice surface realistic.

#### Scenario: Thermodynamics Practice
- **WHEN** the thermodynamics mock test is used
- **THEN** the dataset includes at least 30 illustrated questions (MCQ/multiple-select/true-false) covering laws of thermodynamics, engines, entropy, and phase change so the experience matches previous coverage.
