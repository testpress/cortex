...
- [x] 1.7 Ensure all nested sub-routes (e.g., `:chapterId`, `:threadId`, `player`, `review-analytics`, `:doubtId`, `ask`) also use `parentNavigatorKey: _rootNavigatorKey`.
...
- [x] 2.1 Verify that navigating to a video lesson hides the bottom navigation bar and that pressing back properly disposes the video player, resolving the memory leak and background audio issue.
- [x] 2.2 Verify that navigating to the forum or doubt details page hides the bottom navigation bar and provides an immersive view.
- [x] 2.3 Ensure deep links and sub-routing parameters continue functioning as expected when pushed onto the root navigator.
