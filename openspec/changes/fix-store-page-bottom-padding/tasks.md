...
- [x] 1.3 Wrap price `FittedBox` in `SizedBox(height: 24)` in `ProductCard` to ensure uniform card height across short and long prices.
...
- [x] 2.2 Update widget tests in `packages/courses/test/widgets/store/product_card_test.dart` to verify single-line title with ellipsis and fixed 24dp price row height.
