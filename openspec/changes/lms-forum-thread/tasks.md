...
- [x] 1.1 Implement `threadCommentsProvider` in `packages/forum/lib/providers/forum_providers.dart` to watch replies for a specific thread.
- [x] 1.2 Implement `forumThreadDetailProvider` in `packages/forum/lib/providers/forum_providers.dart` to fetch/watch a single thread's metadata by course and thread ID.

## 2. Core UI Implementation

- [x] 2.1 Create `ForumPostDetailScreen` in `packages/forum/lib/screens/forum_post_detail_screen.dart`.
...
- [x] 2.4 Implement the `StickyReplyInput` with a `flutter_quill` rich text editor and custom toolbar (Bold, Italic, Code, Lists, Images).
