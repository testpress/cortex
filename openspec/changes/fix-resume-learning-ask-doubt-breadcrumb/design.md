...
- Refactor `DoubtContextBadge` to accept a flexible list of `breadcrumbs` instead of hardcoded `courseName` and `chapterName` to support both Lessons and Exam Questions.

**Non-Goals:**
- No changes to external API fetching logic beyond the new `getChapterDetail` endpoint.
