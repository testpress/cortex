...
- **THEN** the system SHALL render a horizontal stacked bar for each subject where green represents strength (correct), red represents weakness (incorrect), and amber (using design.colors.rank1) represents unanswered questions
- **AND** the legend bar SHALL display Strength, Weakness, and Unanswered items in a single horizontal row using design system success, error, and rank1 colors respectively
