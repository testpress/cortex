...
class HttpDataSource implements DataSource {
  ...
  Future<List<QuestionDto>> getQuestions(String attemptId) async {
    final responseData = await performNetworkRequest<Map<String, dynamic>>(
      _dio.get(ApiEndpoints.examQuestions(attemptId)),
      fromJson: (json) => json as Map<String, dynamic>,
    );

    return QuestionDto.parseV3List(responseData);
  }
  ...
  Future<SectionDto> endSection(String attemptId, String sectionOrder) async {
    return performNetworkRequest(
      _dio.put(ApiEndpoints.endSection(attemptId, sectionOrder)),
  ...
  Future<SectionDto> startSection(String attemptId, String sectionOrder) async {
    return performNetworkRequest(
      _dio.put(ApiEndpoints.startSection(attemptId, sectionOrder)),
      fromJson: SectionDto.fromJson,
    );
  }
  ...
  Future<AttemptDto> startAttempt(String attemptId) async {
    return performNetworkRequest(
      _dio.post(ApiEndpoints.startAttempt(attemptId)),
      fromJson: AttemptDto.fromJson,
    );
  }
  ...
  Future<QuizReviewResultDto?> submitAnswer(
    ...
    return performNetworkRequest(
      _dio.patch(
        ApiEndpoints.submitAnswer(attemptId, questionId),
        data: answer.toJson(),
      ),
      fromJson: (data) => QuizReviewResultDto.fromJson(data),
    );
  }
  ...
  Future<AttemptDto> sendHeartbeat(String attemptId) async {
    return performNetworkRequest(
      _dio.put(ApiEndpoints.heartbeat(attemptId)),
      fromJson: AttemptDto.fromJson,
    );
  }
