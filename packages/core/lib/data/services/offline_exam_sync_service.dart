...
class OfflineExamSyncService {
  ...
  /// Sync a specific offline exam download by ID.
  Future<bool> syncExam(int downloadId) async {
    final downloads = await (_db.select(
      _db.offlineExamDownloadsTable,
    )..where((t) => t.id.equals(downloadId))).get();
    if (downloads.isEmpty) return false;
    return _syncSingleDownload(downloads.first);
  }

  /// Sync executor for all pending exams.
  Future<void> syncPendingExams() async {
    ...
    for (final download in pendingDownloads) {
      await _syncSingleDownload(download);
    }
  }

  Future<bool> _syncSingleDownload(
    ...
    try {
      ...
      for (final item in items) {
        ...
        if (item.selectedChoices != null && item.selectedChoices!.isNotEmpty) {
          try {
      ...
      if (isPermanentFailure) {
        debugPrint("Permanent failure. Dropping sync for exam ${download.id}.");
        await _db.deleteDownload(download.id);
      } else {
        // Revert back to PENDING_SYNC for retry on next attempt
        await _db.upsertDownload(
          download
              .toCompanion(false)
