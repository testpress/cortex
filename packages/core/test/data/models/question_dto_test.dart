...
void main() {
  group('QuestionDto', () {
    test(
      'derives correct option ids from answer metadata when explicit ids are absent',
      () {
        ...
        expect(
          question.options.firstWhere((option) => option.id == '102').isCorrect,
          true,
        );
      },
    );
  ...
  group('AnswerDto', () {
    test(
      'preserves selected option ids for review while serializing numeric ids for the API',
      () {
    ...
    test('allows quiz review submissions to override the review flag', () {
      final answer = AnswerDto(questionId: 'q-1', selectedOptions: ['102']);
  ...
  group('QuizReviewResultDto', () {
    ...
    test(
      'parses attempt question response format with nested fields and correct_option_ids',
      () {
        final review = QuizReviewResultDto.fromJson({
          'id': '1021279043',
          'selected_answers': ['2426143'],
          'correct_option_ids': ['2426143'],
          'question': {'id': '659389', 'question_html': 'Let A be a point...'},
        });
        ...
        expect(
          review.result,
          true,
        ); // derived since result was missing but answers match
      },
    );

    test(
      'parses nested options with is_correct flag to identify correct answers',
      () {
        final review = QuizReviewResultDto.fromJson({
          'id': '1021279043',
          'selected_answers': ['2426143'],
