...
void main() {
  ...
  group('AuthRepository', () {
    test(
      'loginWithPassword should call API and save token on success',
      () async {
        // Arrange
        const token = 'fake_token';
        when(
          mockApi.loginWithPassword(username: 'user', password: 'pass'),
    ...
    test(
      'loginWithPassword should rethrow API exceptions and NOT save token',
      () async {
        // Arrange
    ...
    test('logout should call API and clear token even if API fails', () async {
      ...
      when(
        mockApi.logout(authToken: anyNamed('authToken')),
      ).thenThrow(const AuthException('API error'));
